...
#        Log    \n Response = ${resp.json()}    console=yes
#        Should Be Equal As Strings    ${resp.status_code}    200
#        ${status}    Validate Node Provision Status    ${resp.json()}    ${server1_node_name}
...
Assign Interface Ip to node to form Topology
	...
	Log    \n Updating invader topology with data = ${i1_topology} \n
	${resp}  Post Request    platina   ${add_interface}    json=${i1_topology}    headers=${headers}
	Log    \n Status Code = ${resp.status_code}    console=yes
	Log    \n JSON RESP = ${resp.json()}    console=yes
	Should Be Equal As Strings  ${resp.status_code}  200

	Log    \n Updating invader topology with data = ${i2_topology} \n
