...
Pcc-node-summary-add-node-without-BMC-password
    ...
    Should Be Equal As Strings    ${resp.json()['status']}    200
 	...
 	Set Suite Variable    ${node2_id}    ${node2_id}
 	Log    \n Node ${node2_name} ID = ${node1_id}   console=yes
...
Pcc-node-summary-add-node-managed-by-pcc
    ...
    Should Be Equal As Strings    ${resp.status_code}    200
    Should Be Equal As Strings    ${resp.json()['status']}    200
    ${status}    ${node_id}    Validate Node Manage Status    ${resp.json()}    ${node1_name}    false
...
Pcc-Node-summary-DeleteNode
    ...
    &{data}    Create Dictionary    0=${node2_id}
...
Pcc-node-summary-add-node-with-some-features
	...
	# Add Node
	&{data}    Create Dictionary  	Name=${node3_name}  Host=${node3_host_addr}  bmc=${node3_bmc}  bmcPassword=${node3_bmc_pwd}
	${resp}    Post Request    platina    ${add_node}    json=${data}   headers=${headers}
	Log    \n Status code = ${resp.status_code}    console=yes
	Log    \n Response = ${resp.json()}    console=yes
	Should Be Equal As Strings    ${resp.status_code}    200
...
Pcc-Node-Role-create
	...
	# Add Role
	&{data}    Create Dictionary  name=${role1_name}    description=${role1_desc}    templateIDs=${role1_id}
    ...
    Should Be Equal As Strings    ${resp.status_code}    200
    Should Be Equal As Strings    ${resp.json()['status']}    200
 	${status}    ${role_id}    Validate Role    ${resp.json()}    ${role1_name}
...
Pcc-sites-delete-site
    ...
    # Delete Site
    &{data}    Create Dictionary  0=${site2_id}
